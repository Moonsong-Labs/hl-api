...
def test_limit_order_formats_perp_price(api: HLProtocolEVM) -> None:
    ...
    api._metadata.resolve_perp_sz_decimals = MagicMock(return_value=4)
    ...
    api._metadata.resolve_perp_sz_decimals.assert_called_once_with(4)
    api._metadata.resolve_spot_base_sz_decimals.assert_not_called()
...
def test_limit_order_formats_spot_price_when_perp_metadata_missing(api: HLProtocolEVM) -> None:
    ...
    api._metadata.resolve_perp_sz_decimals = MagicMock(return_value=None)
    api._metadata.resolve_spot_base_sz_decimals = MagicMock(return_value=2)
    ...
    assert _captured_price_uint(api) == to_uint64(expected_price, 8)
    api._metadata.resolve_perp_sz_decimals.assert_called_once_with(7)
    api._metadata.resolve_spot_base_sz_decimals.assert_called_once_with(7)
...
def test_limit_order_falls_back_when_metadata_unavailable(api: HLProtocolEVM) -> None:
    ...
    api._metadata.resolve_perp_sz_decimals.assert_called_once_with(9)
    api._metadata.resolve_spot_base_sz_decimals.assert_called_once_with(9)
